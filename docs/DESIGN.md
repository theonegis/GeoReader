...
本文汇总原 README 的架构、交互、渲染、打包和维护说明。科学数据实现与限制另见 [科学数据](SCIENTIFIC_DATA.md)、[验收记录](SCIENTIFIC_TEST_REPORT.md) 和 [图标规范](APP_ICONS.md)。
...
- 栅格只读取当前地图视窗，并根据屏幕像素大小自动使用 GeoTIFF 内部或
  外部 overview（金字塔）；多维变量按当前切片渲染
- 色带、色带方向、拉伸范围和透明度修改后重绘当前视窗，不扫描整幅栅格
...
GDAL 输出当前视窗所需波段的 Float64 数据和 alpha mask，再按单波段色带或
RGB 波段组合进行内存着色。最小值–最大值、2%–98% 裁剪、均值 ±2σ 和
直方图均衡化基于当前视窗值。NoData 与非有限值保持透明。

当前实现没有跨帧的 256 MiB LRU 浮点缓存；改变样式会重新读取当前视窗。
早期文档中描述的该缓存属于未落地设计，不能作为当前性能保证。
多维数据预览、曲线和地图读取共享锁，避免 NetCDF/HDF5 底层库并发访问崩溃。
...
而不是等待鼠标停止或单击；仅在坐标变化时查询。查询以只读方式打开数据，并与科学数据读取共享锁。查询结果包含所有可见栅格的像素
行列号和各波段值。
...
- 后台结果使用值语义结构体，GDAL 多维数组使用受控的共享句柄；
- `std::clamp`、结构化绑定、`[[nodiscard]]`、RAII 和不可变快照用于
  减少边界错误与隐式所有权；
- 科学数据读取和句柄释放由统一递归锁保护，QML/GUI 状态只在主线程更新；
...
```bash
QT_QPA_PLATFORM=offscreen build/GeoReaderIconTool "$PWD"
```
...
应用图标的跨平台尺寸与生成方式见 [图标维护说明](APP_ICONS.md)。
