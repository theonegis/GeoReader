...
Dataset open(const QString &path) {
  ...
  const auto requestedCrs = s.value("crs").toString();
  if (!requestedCrs.isEmpty()) {
    OGRSpatialReference overrideCrs;
    if (overrideCrs.SetFromUserInput(requestedCrs.toUtf8().constData()) == OGRERR_NONE) {
      overrideCrs.SetAxisMappingStrategy(OAMS_TRADITIONAL_GIS_ORDER);
      vrt->SetSpatialRef(&overrideCrs);
    }
  }
